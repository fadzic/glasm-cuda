...
double *d_lookup_oy;
double *d_lookup_step_y;
unsigned *d_lookup_rows;
unsigned *d_lookup_columns;
unsigned *d_counter;
int *d_redings_size;
position *d_pos;
...
void setDebugParameters(bool udraw_lookup, bool udraw_individual,
		bool udraw_generations, unsigned uverbose_level, double umap_size_x,
		double umap_size_y, std::string umap_filename,
		std::string ulookup_bitmap, std::string uvalid_bitmap) {
...
void setGeneticParameters(int unbitx, int unbity, int unbitrot, int upopsize,
		int umaxruns, int umaxgen, double upcross, double upmutation) {
	nbitx = unbitx;
	nbity = unbity;
	nbitrot = unbitrot;
	popsize = upopsize;
...
void setLookupTableParameters(int ulookup_rows, int ulookup_columns,
		double ulookup_size_x, double ulookup_size_y, double ulookup_centerx,
		double ulookup_centery, double ucorr_distance) {
...
*/
	...
	CUDA_SAFE_CALL(cudaMalloc((void **)&d_lookup_oy, sizeof(double)));
	CUDA_SAFE_CALL(cudaMemcpy(d_lookup_oy, &lookup_oy, sizeof(double), cudaMemcpyHostToDevice));

	CUDA_SAFE_CALL(cudaMalloc((void **)&d_lookup_step_x, sizeof(double)));
	CUDA_SAFE_CALL(cudaMemcpy(d_lookup_step_x, &lookup_step_x, sizeof(double), cudaMemcpyHostToDevice));

	CUDA_SAFE_CALL(cudaMalloc((void **)&d_lookup_step_y, sizeof(double)));
	CUDA_SAFE_CALL(cudaMemcpy(d_lookup_step_y, &lookup_step_y, sizeof(double), cudaMemcpyHostToDevice));

	CUDA_SAFE_CALL(cudaMalloc((void **)&d_lookup_rows, sizeof(unsigned)));
	CUDA_SAFE_CALL(cudaMemcpy(d_lookup_rows, &lookup_rows, sizeof(unsigned), cudaMemcpyHostToDevice));

	CUDA_SAFE_CALL(cudaMalloc((void **)&d_lookup_columns, sizeof(unsigned)));
	CUDA_SAFE_CALL(cudaMemcpy(d_lookup_columns, &lookup_columns, sizeof(unsigned), cudaMemcpyHostToDevice));
}
...
void setSearchArea(double uMinX, double uMaxX, double uMinY, double uMaxY,
		double uMinRot, double uMaxRot) {
	MaxX = uMaxX;
	MinX = uMinX;
	MaxY = uMaxY;
	MinY = uMinY;
...
void setCenterOfSearchArea(position pos) {   // todo
}
...
// cresci
	...
	while (i1 != ScanRef.readings.end()) {
		...
		double X = ScanRef.pos.x
				+ i1->distance * cos(ScanRef.pos.rot + i1->angle);
		double Y = ScanRef.pos.y
				+ i1->distance * sin(ScanRef.pos.rot + i1->angle);

		// discretizzazione
		unsigned icx = (unsigned int) (floor(
				(X - lookup_ox) / lookup_step_x + 0.5)); // discretizzo X+lookup_ox cioe X espresso in coord locali nel frame
		unsigned icy = (unsigned int) (floor(
				(Y - lookup_oy) / lookup_step_y + 0.5)); // -
...
void initialize_radial_gradient(double RGmean, double RGvariance,
		double RGextension, double RGweight, double RGscale,
		double RGbitmapsize) {   // prepare radial gradient bitmap
...
//std::cout << "bmp" << RGmean << "," << RGvariance << "," << RGextension << "," << RGweight << "," << RGscale << "," <<  RGbitmapsize << std::endl;
	try {   // allocazione memoria per array (RadialGradientBitmap)
...
// inizializzazione array di lookup
	for (unsigned i = 0; i < NX; i++)
		for (unsigned j = 0; j < NX; j++) {
			double d = sqrt((i - NX2) * (i - NX2) + (j - NX2) * (j - NX2))
					/ RGextension;
			double t1 = (d - RGmean) / RGvariance;
			double t2 = -0.5 * t1 * t1;
			RGbmp[i][j] = (unsigned) (exp(t2) / (RGvariance * sqrt(2 * M_PI))
					* RGweight);
...
// cresci
	...
	//copy to CUDA global memory
	cudaMalloc((void **)&d_lookup, lookup_rows*lookup_columns*sizeof(char));
	cudaMemcpy(d_lookup, lookup, lookup_rows*lookup_columns*sizeof(char), cudaMemcpyHostToDevice);
}
//---------------------------------------------------------------------------
...
//    GLASM lookup table parameters
		{
	sga_parameters(nbitx, nbity, nbitrot, popsize, maxruns, maxgen, pcross,
			pmutation);    // set genetic parameters
